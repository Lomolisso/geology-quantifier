...
class PanoramicExtraction(object):
    ...
    def _draw_circles_and_lines(self) -> None:
        ...
        for i in range(self.TOTAL_VERTEXES):
            ...
            v2 = self.vertex_data[(i+1) % self.TOTAL_VERTEXES]
    ...
    def check_circle_movement(self, x: int, y: int) -> None:
        ...
        """
        for v in range(self.vertex_data):
...
class SampleExtractor(object):
    ...
    def reset_vertexes_pos(self) -> None:
        ...
        d = np.array((self.image_size[1]//2-image_cols//2, self.image_size[0]//2-image_rows//2))
        self.vertex_data["vertex_1"] = np.array((0, 0))+d
        self.vertex_data["vertex_2"] = np.array((0, image_rows))+d
        self.vertex_data[f"vertex_{3 + step}"] = np.array((image_cols, image_rows))+d
        self.vertex_data[f"vertex_{4 + step}"] = np.array((image_cols, 0))+d

        if self.total_vertexes == 6:
            self.vertex_data["vertex_3"] = np.array((image_cols//2, image_rows))+d
            self.vertex_data["vertex_6"] = np.array((image_cols//2, 0))+d
